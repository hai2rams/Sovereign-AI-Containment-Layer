...
/** True when live T3N session calls are possible (API key present). */
export function isT3Configured(config: T3AdapterConfig): boolean {
  return Boolean(config.apiKey);
}

/** True when a registered contract id is available for future anchor writes. */
export function isT3ContractReady(config: T3AdapterConfig): boolean {
  return isT3Configured(config) && Number.isInteger(config.contractId) && config.contractId > 0;
}
